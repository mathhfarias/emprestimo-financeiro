...
export default function Loans() {
  ...
  async function handleSubmit(payload) {
    try {
      if (!hasPermission(PERMISSIONS.LOAN_CREATE)) {
        setError('Você não tem permissão para cadastrar empréstimos.');
        return;
      }
  ...
  const columns = [
    ...
    },
    {
      ...
      header: 'Juros',
      render: (row) => formatPercent(row.monthly_interest_rate),
    },
    {
    ...
    },
    {
      ...
      render: (row) => formatDate(row.first_due_date),
    },
    {
      key: 'status',
      header: 'Status',
      ...
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Cadastre empréstimos e acompanhe o status de cada contrato.
          </p>
        </div>
