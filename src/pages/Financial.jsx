...
export default function Financial() {
  ...
  const enrichedOverdue = useMemo(() => {
    return overdue.map((installment) => {
      const lateCalculation = calculateLateInstallment({
        ...
        lateFeeRate: installment.loan?.late_fee_rate,
        dailyLateInterestRate: installment.loan?.daily_late_interest_rate,
      });
  ...
  const totals = useMemo(
    () => ({
      ...
      overdueOriginal: sumBy(
        enrichedOverdue,
        (installment) => installment.amount
      ),
      overdueUpdated: sumBy(
        enrichedOverdue,
        (installment) => installment.updatedAmount
      ),
    }),
    [payments, enrichedOverdue]
  );
  ...
  const overdueColumns = [
    {
      key: 'client',
      header: 'Cliente',
      render: (row) => row.loan?.client?.name || '-',
    },
    {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Relatórios de recebimentos, valores em aberto e atrasados.
          </p>
        </div>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <FinancialCard
          title="Atrasado original"
          value={formatCurrency(totals.overdueOriginal)}
        />

        <FinancialCard
          title="Atrasado atualizado"
          value={formatCurrency(totals.overdueUpdated)}
        />
      </section>
      ...
      <section className="space-y-3">
        ...
        <SimpleTable
          ...
          data={enrichedOverdue}
          emptyTitle="Nenhuma parcela atrasada"
        />
      </section>
    </div>
  );
