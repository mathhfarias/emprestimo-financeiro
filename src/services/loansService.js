...
export async function createLoanWithInstallments(payload) {
  ...
  const loanPayload = {
    ...
    late_fee_rate: Number(payload.late_fee_rate || 0),
    daily_late_interest_rate: Number(payload.daily_late_interest_rate || 0),
...
function validateLoan(payload) {
  ...
  if (Number(payload.principal_amount) <= 0) {
    throw new Error('Informe o valor emprestado.');
  }

  if (Number(payload.monthly_interest_rate) < 0) {
    throw new Error('Informe a taxa de juros.');
  }

  if (Number(payload.installments_count) <= 0) {
    throw new Error('Informe a quantidade de parcelas.');
  }

  if (Number(payload.late_fee_rate) < 0) {
    throw new Error('Informe uma multa por atraso válida.');
  }

  if (Number(payload.daily_late_interest_rate) < 0) {
    throw new Error('Informe um juros diário por atraso válido.');
  }
